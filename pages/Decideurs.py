...
if st.session_state.user['role'] not in ['Administrateur']:
    st.error("Accès réservé aux décideurs")
    st.stop()
...
st.info("Vérifiez tous les indicateurs avant de valider l'emploi du temps final.")
...
with col1:
    if st.button("Valider l'Emploi du Temps Final", use_container_width=True, type="primary"):
        ...
        <div class="success-banner">
            Emploi du temps validé avec succès
        </div>
        """, unsafe_allow_html=True)
        st.balloons()
...
if examens_df.empty:
    st.info("Aucun examen planifié. Contactez le service planification.")